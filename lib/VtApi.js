export default class VasttrafikAPI {
  ...
  async getTrip(fromId, toId) {
    if ((fromId && !toId) || (!fromId && toId)) {
      ...
      var { id: fromId, name: fromName } = await this.getNearbyStop(
        await this.fetchGeoLocation()
      );
    }
    const rNumber = /^\d+$/;
    if (!rNumber.test(fromId))
      var { id: fromId, name: fromName } = await this.getStop(fromId);
    if (!rNumber.test(toId))
      var { id: toId, name: toName } = await this.getStop(toId);
